import {CustomRequestInit} from "@/app/services/network.types";



async function networkInstance(endpoint: string, options?: CustomRequestInit){
  ...
  const response = await fetch(`${process.env.NEXT_PUBLIC_API_ENDPOINT}/${processedEndpoint}`,options)
    if(!response.ok){
