...
import WarnIcon from "@/app/_components/input/icons/warnIcon";
...
const TextInput = ({
  ...
  return (
    <div className={`flex items-center gap-[24px] sm:flex-col sm:gap-[12px] md:gap-[12px] ${className}`}>
      ...
      <div className={"flex w-full flex-col gap-[8px] border-[1px] border-gray-100 rounded-primary"}>
        {/*인풋*/}
        <div
          className={
        ...
        >
          <input
            ...
            className={"h-full w-full bg-white text-black outline-none text-menu font-[400]"}
            {...(maxLength && { maxLength })}
        ...
        {validation && <p className={"text-body-s text-alert-normal sm:text-body md:text-body"}>{validationMessage}</p>}
      </div>
    </div>
  );
};
