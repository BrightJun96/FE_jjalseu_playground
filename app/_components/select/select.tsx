...
const Select = ({ options, handleOptionChange,label }: SelectProps) => {
  ...
  return (
    <div ref={ref} className="relative sm:w-full md:w-full ">
        <div className={"flex flex-col gap-2"}>
          {/*라벨*/}
          {label&&<label className="text-title3Normal px-2">{label}</label>}
          {/*선택버튼*/}
          <button
            type="button"
          ...
          >
            <span className={"text-body"}>{selectedOption.text}</span>
            <DropIcon rotate={isClicked ? 180 : 0} />
          </button>
        </div>
      {/*옵션목록*/}
      <div
        ...
        <div className="py-1" role="none">
          <ul>
            {options.map((option, index) => (
              ...
              >
                <button
                  className={"flex h-fit w-full justify-between px-[12px] py-[8px]"}
                  onClick={() => {
